...
AegisGate can be deployed in minutes using Docker, Kubernetes, or bare metal. The **Guided Setup** features (v4.3.0+) make deployment even simpler — use the setup wizard to auto-detect your environment and generate a validated config, or pick a deploy profile that matches your use case.
...
v4.3.0 introduces CNN-BiLSTM neural network detection. To enable it, mount the ONNX Runtime shared library:
...
curl -sL https://github.com/aegisgatesecurity/aegisgate-platform/releases/download/v4.3.0/aegisgate-platform-linux-amd64 -o aegisgate-platform
