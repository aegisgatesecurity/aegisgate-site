...
|---|---|---|---|---|
| GV.OC-01 | Organizational mission is informed by cybersecurity risk | AegisGate's mission — securing AI infrastructure — is the primary driver of product security architecture. Every platform control (1,457 automated controls across 31 frameworks) is designed to translate cybersecurity risk requirements into enforceable policy. | 2 | ✅ Implemented |
...
| GV.OC-03 | Cybersecurity risk management expectations and requirements are established | Compliance engine enforces risk requirements via 1,457 automated controls across NIST CSF, SOC 2, HIPAA, PCI-DSS, and 20 additional frameworks. Risk requirements are codified as policy, not advisory guidance. | 3 | ✅ Implemented |
...
| GV.RM-03 | Cybersecurity risk response is determined | Automated risk response via the compliance engine: violations trigger real-time alerts, enforcement actions (block, redact, rate-limit), and audit logging. 216 detection patterns enable risk-informed response. | 3 | ✅ Implemented |
| GV.RM-04 | Risk management is integrated into business processes | Product security is embedded in the development lifecycle. Compliance checks are automated, not bolted on — 1,457 automated controls run continuously as part of platform operation. | 2 | ⚠️ Partial |
...
| ID.AM-02 | Asset vulnerabilities are identified and managed | Compliance engine runs 1,457 automated controls including vulnerability-oriented controls across 31 frameworks. 216 detection patterns identify exposure risks in real time (secrets, PII/PHI, prompt injection vectors, data exfiltration paths). | 3 | ✅ Implemented |
...
| NIST ID | Subcategory | AegisGate Implementation | Tier | Status |
|---|---|---|---|---|
| ID.RA-01 | Risk assessment process is established | AG-RISK-GOVERNANCE defines the risk assessment methodology. The compliance engine operationalizes risk assessment through continuous automated evaluation of 1,457 control checks across 31 frameworks. | 3 | ✅ Implemented |
| ID.RA-02 | Threat sources are identified | 216 detection patterns covering: secrets exposure, PII/PHI leakage, prompt injection attacks, data exfiltration attempts, and tool-use abuse. Detection patterns are continuously updated and validated against emerging threat intelligence. | 3 | ✅ Implemented |
...
| ID.RA-05 | Threat and vulnerability information is used | Detection patterns are informed by real-world threat data. 216 patterns reflect known attack vectors in AI infrastructure, including OWASP LLM Top 10 coverage. | 3 | ✅ Implemented |
...
| DE.CM-02 | Service and network behavior is monitored | 216 detection patterns continuously analyze traffic behavior: anomaly detection for secrets exposure, PII/PHI leakage, prompt injection attempts, and data exfiltration. Per-tier RPM rate limiting monitors for abuse patterns. | 3 | ✅ Implemented |
...
| DE.CM-07 | Vulnerability management is monitored | Compliance engine continuously evaluates 1,457 control checks, including vulnerability-oriented detections. Failed checks trigger alerts and remediation guidance. | 3 | ✅ Implemented |
...
| RS.MA-02 | Incident response plan is tested | Incident response procedures validated through formal SLA targets ([/security/incident-response-sla/](/security/incident-response-sla/)) and compliance engine's 1,457+ automated checks. Platform controls are continuously tested in production through real threat detection. | 3 | ✅ Addressed |

#### RS.AN — Incident Analysis

| NIST ID | Subcategory | AegisGate Implementation | Tier | Status |
|---|---|---|---|---|
| RS.AN-01 | Notifications are received and investigated | All gateway events are logged in hash-chained audit logs. Anomalous events trigger real-time alerts. 216 detection patterns ensure comprehensive coverage of AI-specific threat vectors. | 3 | ✅ Implemented |
...
| RC.RE-02 | Recovery is validated | Hash-chained audit logs provide post-recovery integrity verification. Compliance engine re-validates all 1,457 control checks after recovery. | 2 | ✅ Implemented |
...
| Advanced persistent threats | Medium | 216 detection patterns, hash-chained audit logs | Medium — evolving threat landscape |
