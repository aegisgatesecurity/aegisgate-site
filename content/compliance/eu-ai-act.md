...
| **AegisGate Implementation** | AegisGate does not train, fine-tune, or deploy any AI model. No training, validation, or testing datasets are used. Data governance requirements under Article 10 are not applicable. AegisGate's detection patterns (223 rules for prompt injection, PII/PHI, secrets, data exfiltration) are authored by AegisGate's security research team, reviewed through a controlled release process, and versioned in AegisGate's configuration. Pattern updates are delivered through signed release channels. |
